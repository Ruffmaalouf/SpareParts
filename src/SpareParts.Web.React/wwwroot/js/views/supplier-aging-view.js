...
export function SupplierAgingView({ api }) {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    setStatus("Loading supplier aging...");
    try {
      const data = await api.get("/api/suppliers/aging");
      setRows(Array.isArray(data) ? data : []);
      setStatus("Aging report loaded.");
    } catch (error) {
      setStatus(error.message || "Could not load supplier aging.");
    } finally {
      setIsLoading(false);
    }
  }, [api]);
  ...
  const visibleRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      [row.supplierName, row.phone].some((val) => String(val || "").toLowerCase().includes(q))
    );
  }, [rows, search]);
  ...
  return h("section", { className: "screen" },
    h(PageHeader, {
      ...
      subtitle: "Outstanding payables by age bucket — track overdue amounts owed to suppliers.",
      action: h("button", { className: "secondary-button", onClick: load, disabled: isLoading }, "Refresh")
    }),
    h("section", { className: "module-summary" },
      h("div", null, h("span", null, "Suppliers with Balances"), h("strong", null, visibleRows.length)),
      h("div", null, h("span", null, "Overdue 30+ days"), h("strong", null, over30)),
      h("div", null, h("span", null, "Overdue 90+ days"), h("strong", null, over90)),
      h("div", null, h("span", null, "Total Payable"), h("strong", null, money(totals.totalBalance, "USD")))
    ),
    h(StatusLine, { status }),
    ...
    h("section", { className: "table-panel" },
      h(DataTable, {
        columns: [
          { key: "supplier", label: "Supplier", render: (row) => h("strong", null, row.supplierName) },
