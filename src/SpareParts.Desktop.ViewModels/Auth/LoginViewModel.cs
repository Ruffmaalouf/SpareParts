    ...
    public class LoginViewModel : INotifyPropertyChanged
    {
        ...
        private async void ExecuteLoginAsync(string password)
        {
            ...
            try
            {
                ...
                catch (Exception ex)
                {
                    ...
                    var startupDetails = FlattenExceptionMessages(ex);
                    var logPath = TryWriteStartupExceptionLog(ex);
                    ErrorMessage = logPath == null
                        ? $"Login succeeded, but the main window could not open: {startupDetails}"
                        : $"Login succeeded, but the main window could not open: {startupDetails} See log: {logPath}";
                }
            }
        ...
        private static string FlattenExceptionMessages(Exception ex)
        {
            ...
            for (var current = ex; current != null; current = current.InnerException)
            {
                ...
                if (messages.Any(existing => string.Equals(existing, message, StringComparison.Ordinal)))
                {
                    continue;
                }
            ...
            return messages.Count == 0 ? ex.ToString() : string.Join(" -> ", messages);
        }

        private static string? TryWriteStartupExceptionLog(Exception ex)
        {
            try
            {
                ...
                var logPath = Path.Combine(
                    logsDirectory,
                    $"startup-error-{DateTime.Now:yyyyMMdd-HHmmss}.log");
