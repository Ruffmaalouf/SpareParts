...
{
    public sealed class ActivityLogViewModel : INotifyPropertyChanged
    {
        private readonly ICrudApiClient _crudApi;
        private bool _isLoading;
        private string _status = "Load activity log.";
        private Brush _statusBrush = Brushes.LightGray;

        public ActivityLogViewModel(ICrudApiClient crudApi)
        {
            _crudApi = crudApi;
        ...
        public ObservableCollection<ActivityLogDto> Items { get; } = new();
        ...
        public async Task LoadAsync()
        {
            ...
            try
            {
                var items = await _crudApi.GetAllAsync<ActivityLogDto>("api/activity-log");
                Items.Clear();
