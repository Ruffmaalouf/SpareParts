...
{
    public sealed class SupplierAgingViewModel : INotifyPropertyChanged
    {
        private readonly ICrudApiClient _crudApi;
        private bool _isLoading;
        private string _status = "Load supplier aging.";
        private Brush _statusBrush = Brushes.LightGray;

        public SupplierAgingViewModel(ICrudApiClient crudApi)
        {
            _crudApi = crudApi;
        ...
        public ObservableCollection<SupplierAgingDto> Items { get; } = new();
        ...
        {
            IsLoading = true;
            Status = "Loading supplier aging...";
            StatusBrush = Brushes.LightGray;
            try
            {
                var items = await _crudApi.GetAllAsync<SupplierAgingDto>("api/suppliers/aging");
                Items.Clear();
                foreach (var item in items) Items.Add(item);
                Status = $"{items.Count} supplier balance row(s) loaded.";
