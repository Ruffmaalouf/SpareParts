...
{
    public sealed class ExpiryAlertsViewModel : INotifyPropertyChanged
    {
        private readonly ICrudApiClient _crudApi;
        private bool _isLoading;
        private string _status = "Load expiry alerts.";
        private Brush _statusBrush = Brushes.LightGray;

        public ExpiryAlertsViewModel(ICrudApiClient crudApi)
        {
            _crudApi = crudApi;
        ...
        public ObservableCollection<PartExpiryAlertDto> Items { get; } = new();
        ...
        {
            IsLoading = true;
            Status = "Loading expiry alerts...";
            StatusBrush = Brushes.LightGray;
            try
            {
                var items = await _crudApi.GetAllAsync<PartExpiryAlertDto>("api/parts/expiry/alerts");
                Items.Clear();
