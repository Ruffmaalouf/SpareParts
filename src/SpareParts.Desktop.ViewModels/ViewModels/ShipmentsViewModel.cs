...
{
    public sealed class ShipmentsViewModel : INotifyPropertyChanged
    {
        private readonly ICrudApiClient _crudApi;
        private bool _isLoading;
        private string _status = "Load shipments.";
        private Brush _statusBrush = Brushes.LightGray;

        public ShipmentsViewModel(ICrudApiClient crudApi)
        {
            _crudApi = crudApi;
        ...
        public ObservableCollection<ShipmentDto> Items { get; } = new();
        ...
        {
            IsLoading = true;
            Status = "Loading shipments...";
            StatusBrush = Brushes.LightGray;
            try
            {
                var items = await _crudApi.GetAllAsync<ShipmentDto>("api/shipments");
                Items.Clear();
