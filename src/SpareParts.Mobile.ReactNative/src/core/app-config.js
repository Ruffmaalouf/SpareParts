...
// identities) to SpareParts.Web.React/wwwroot/js/core/config.js's `wpfThemes` array. Mobile
// (React Native runtime) and web (browser-served static JS) cannot literally share one JS module,
// so this is the canonical roster expressed in two places on purpose — if you edit one, edit the
// other to match. WPF's theme files are the ultimate source of the hex values.
const wpfThemes = [
  {
    key: "apex",
    name: "Apex",
...
const featureModules = [
  ...
  { key: "mechanic-desk", label: "Mechanic Desk", title: "Mechanic Desk", endpoint: "/api/repair-orders", capabilities: ["Track mechanic orders", "Send quote-ready requests", "Accept winning mechanic quotes"] },
  ...
  { key: "halfcut", label: "Half-Cut Showcase", title: "Half-Cut Showcase", endpoint: "/api/half-cuts", capabilities: ["List half-cut inventory", "Post new half-cut offers", "Manage buyer claims and confirmations"] },
  { key: "car-crush", label: "CarCrush", title: "CarCrush", endpoint: "/api/car-crush/generate", commandOnly: true, capabilities: ["Decode donor vehicles", "Generate dismantling checklists", "Create salvage listing packs"] },
  ...
  { key: "qr-tag", label: "QR Tag System", title: "QR Tag System", endpoint: "/api/qr-tag/{id}", commandOnly: true, capabilities: ["Generate QR labels for parts", "Fetch printable QR data", "Support scan-driven retrieval"] },
  { key: "part-genealogy", label: "Part Genealogy", title: "Part Genealogy", endpoint: "/api/part-genealogy/{id}", commandOnly: true, capabilities: ["Trace part lifecycle events", "Record chain-of-custody notes", "Explain donor and movement history"] },
  ...
  { key: "new-vs-used", label: "New vs Used", title: "New vs Used", endpoint: "/api/new-vs-used/{id}", commandOnly: true, capabilities: ["Compare new and used pricing", "Override indexed prices", "Support customer trade-off conversations"] },
