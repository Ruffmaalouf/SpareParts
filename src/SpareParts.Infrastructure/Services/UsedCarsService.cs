...
public sealed class UsedCarsService
{
    ...
    /// <summary>
    /// Used cars that have been received but do not yet have an automated teardown/parts
    /// plan generated by the Intake &amp; Teardown Agent.
    /// </summary>
    public IEnumerable<PendingTeardownCarDto> GetPendingTeardownCars()
    {
        var tenantId = _tenantContext.TenantId;
        using var conn = _factory.CreateConnection();
        return conn.Query<PendingTeardownCarDto>(
            @"SELECT uc.Id,
                     cb.Name AS Make,
                     cm.Name AS Model,
                     uc.ModelYear,
              ...
              WHERE uc.IsReceived = 1
                AND uc.TeardownPlanGeneratedAt IS NULL
    ...
    /// <summary>
    /// Marks a used car as processed by the Intake &amp; Teardown Agent so it is not picked up again.
    /// </summary>
    public void MarkTeardownPlanGenerated(int usedCarId, int partsCreated)
    {
        using var conn = _factory.CreateConnection();
        conn.Execute(
            @"UPDATE dbo.UsedCars
              SET TeardownPlanGeneratedAt = SYSUTCDATETIME(),
                  TeardownPlanPartsCreated = @PartsCreated
              WHERE Id = @Id;",
            new { Id = usedCarId, PartsCreated = partsCreated });
    }
