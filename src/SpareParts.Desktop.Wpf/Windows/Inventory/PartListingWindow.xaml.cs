...
{
    public partial class PartListingWindow : Window
    {
        private readonly ManagementCoordinator _coordinator;
        private readonly PartWorkspaceRequest _request;

        public PartListingWindow(ManagementCoordinator coordinator, PartWorkspaceRequest request)
        {
            InitializeComponent();
            _coordinator = coordinator;
            _request = request;
            Title = $"Listing Package — {request.PartName}";
        ...
        private async System.Threading.Tasks.Task LoadPackageAsync()
        {
            try
            {
                var package = await _coordinator.GetPartListingPackageAsync(_request.PartId);
                Apply(package);
            }
            catch (Exception ex)
            {
        ...
        private void Apply(PartListingPackageDto package)
        {
            TitleTextBox.Text = package.Title;
            DescriptionTextBox.Text = package.Description;
            PhotoCountText.Text = $"{package.PhotoCount} photo(s) available in the donor car's gallery — attach them to the listing after pasting the text above.";
